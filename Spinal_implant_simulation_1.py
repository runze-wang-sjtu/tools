...
path = '/home/gdp/data/spine_large/VerSeg_complete/VerSeg/label_1'
...
def generate_implant(seg_data,path,file):
...
for root, _, files in os.walk(path):
    for file in files:
        seg_data, seg_img = load_nifti(os.path.join(path,file))
        generate_implant(seg_data,path=path,file=file)
#####   debug
